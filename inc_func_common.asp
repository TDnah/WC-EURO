...
Function GetUserOnline
	...
	strSql = "SELECT M_NAME FROM " & strMemberTablePrefix & "MEMBERS WHERE IsOnline = -1 AND M_LASTHEREDATE >= '" & DateToStr(DateAdd("n", -10, now)) & "' ORDER BY M_LASTHEREDATE"
	set rs = Server.CreateObject("ADODB.Recordset")
	rs.open strSql, my_Conn
	while not rs.EOF
		strUserOnline = strUserOnline & rs("M_NAME") & ", "
		rs.movenext
	wend
	if strUserOnline <> "" then
		strUserOnline = left(strUserOnline, len(strUserOnline)-2)
	end if
	GetUserOnline = strUserOnline
end Function
