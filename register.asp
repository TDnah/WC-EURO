...
if strProhibitNewMembers <> "1" then
	...
	elseif Request.QueryString("actkey") <> "" and lcase(strEmail) = "1" and strEmailVal = "1" then
		...
		elseif strComp(key,rsKey("M_KEY")) <> 0 then
			...
			'Dung Add for GAMES
            strSql="INSERT INTO Games (MatchID, UserID) SELECT MatchID,'" & chkString(rsKey("M_NAME"),"SQLString") & "' As UserID FROM Matchs"
            my_Conn.Execute (strSql),,adCmdText + adExecuteNoRecords
            'End Dung Add for GAMES
	...
	'#####################################################################################
		...
		if Err_Msg = "" then
            ...
            'Dung Add for GAMES
            strSql="INSERT INTO Games (MatchID, UserID) SELECT MatchID,'" & chkString(trim(Request.Form("Name")),"SQLString")  & "' As UserID FROM Matchs"
            my_Conn.Execute (strSql),,adCmdText + adExecuteNoRecords
            'End Dung Add for GAMES
